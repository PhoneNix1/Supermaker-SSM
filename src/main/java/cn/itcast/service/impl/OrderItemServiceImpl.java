...
import cn.itcast.dao.OrderItemMapper;
...
public class OrderItemServiceImpl implements OrderItemService {
    ...
    public int addBoughtOrderItem(String shoppingNumStr, OrderItem orderItem){
        // 判断商品ID是否重复，重复更新，不重复直接插入
        OrderItem commodity = orderItemMapper.getOrderCommodityID(shoppingNumStr,orderItem.getCommodity_id());
        // 查询库存表中的数据
        Commodity Stock = commodityMapper.getCommodityID(orderItem.getCommodity_id());
        // 如果查到相同的值，购物车表中的数据就进行累加
        if(commodity !=null) {
            ...
            orderItemMapper.updateOrderItem(id, count, total);
        }
    ...
    public List<OrderItem> getOrderChecked(String shoppingNum){
       return orderItemMapper.getOrderChecked(shoppingNum);
    }
    @Override
    // 现金结账-更新购物车表数量，顺便把结账状态改为1
    public int updateOrderStock(int id,int count){
        return orderItemMapper.updateOrderStock(id,count);
    }
    @Override
    //查询购物车结账的数据
    public List<OrderItem> getOrderYChecked(String shoppingNum){
        return orderItemMapper.getOrderYChecked(shoppingNum);
    }
}
